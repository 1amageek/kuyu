...
public struct ImuRateDampingDriveCut: CutInterface {
    ...
    public mutating func update(samples: [ChannelSample], time: WorldTime) throws -> CutOutput {
        ...
        let drives = try [
            DriveIntent(index: DriveIndex(0), activation: thrusts.f1),
            DriveIntent(index: DriveIndex(1), activation: thrusts.f2),
            DriveIntent(index: DriveIndex(2), activation: thrusts.f3),
            DriveIntent(index: DriveIndex(3), activation: thrusts.f4),
        ]

        return .driveIntents(drives)
    }
